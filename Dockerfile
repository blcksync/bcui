...
# RUN cd /home/$ETH_USER; git clone --depth 1 https://github.com/python-telegram-bot/python-telegram-bot --recursive && \
COPY ./python-telegram-bot /home/$ETH_USER/python-telegram-bot

RUN cd /home/$ETH_USER/python-telegram-bot ; git submodule update --init --recursive ; python setup.py install ; \
    chown -R $ETH_USER:$ETH_GID $CONDA_DIR ; \
    chown -R $ETH_USER:$ETH_GID $HOME ; \
    update-permission $HOME && \
    update-permission $CONDA_DIR
